...
int main (int ac, char **av) {
    ...
    uint64_t *eviction_buffer = (uint64_t *)malloc(L3_SIZE);
    ...
    for (int i=0; i<SAMPLES; i++){
        tmp = target_buffer[0];
        for(int k = 0; k < 8; k++)for(int j = 0; j < L1_SIZE; j+=8) tmp = eviction_buffer[j];
	lfence();
        // Step 2: measure the access latency
    ...
    for (int i=0; i<SAMPLES; i++){
        tmp = target_buffer[0];
        for(int k = 0; k < 8; k++)for(int j = 0; j < L2_SIZE; j+=8) tmp = eviction_buffer[j];
	lfence();
        // Step 2: measure the access latency
